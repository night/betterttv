...
function createCustomTimeout() {
  const container = document.createElement('div');
  container.id = CUSTOM_TIMEOUT_ID;
...
function handleTimeoutClick(e, messageId) {
  const customTimeout = document.querySelector(`#${CUSTOM_TIMEOUT_ID}`);
  if (customTimeout == null || e.button === mouseButtons.RightClick) return;

  if (customTimeout.matches(':hover')) {
    let command;
    let duration;
...
function openCustomTimeout(target, messageId) {
  if (document.querySelector(`#${CUSTOM_TIMEOUT_ID}`) != null) return;
  ...
  const customTimeout = createCustomTimeout();
  document.body.appendChild(customTimeout);

  customTimeout.style.top = `${
    target.getBoundingClientRect().top + target.offsetHeight / 2 - customTimeout.offsetHeight / 2
  }px`;
  customTimeout.style.left = `${
    chatRoom.getBoundingClientRect().left - customTimeout.offsetWidth + chatRoom.offsetWidth - 20
  }px`;
  ...
  customTimeout.addEventListener('mousemove', handleMouseMove);
  customTimeout.addEventListener('mousedown', (e) => handleTimeoutClick(e, messageId));
}
...
function handleClick(e) {
  ...
  const msgObject = twitch.getChatMessageObject(chatLine);
  if (!msgObject) {
    return;
  }
...
class ChatCustomTimeoutsModule {
  ...
  loadClickHandler() {
    ...
    off('click', CHAT_ROOM_SELECTOR, handleTimeoutClick);
    on('click', CHAT_ROOM_SELECTOR, handleTimeoutClick);
  }
}
