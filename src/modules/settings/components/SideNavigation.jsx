...
function SideNavigation({open, setOpen}) {
  ...
  // Accordion: only one category is open at a time. Starts on the first category; clicking the
  // open one closes it (leaving none open).
  const [openCategory, setOpenCategory] = useState(() => categorizedGroups[0]?.id ?? null);
  const toggleCategory = useCallback((categoryId) => {
    setOpenCategory((prev) => (prev === categoryId ? null : categoryId));
  }, []);

  const categoryLabels = {
    emotes: formatMessage({defaultMessage: 'Emotes'}),
    chat: formatMessage({defaultMessage: 'Chat'}),
    moderation: formatMessage({defaultMessage: 'Moderation'}),
    channel: formatMessage({defaultMessage: 'Channel'}),
    interface: formatMessage({defaultMessage: 'Interface'}),
