...
import {SettingPanelIds} from './stores/SettingStore';
...
// Icon for the trailing "Other" group that catches any panel not assigned to a category.
const OTHER_CATEGORY_ICON = faEllipsis;
...
export function groupSettingsByCategory(settings) {
  ...
  for (const category of SETTING_CATEGORIES) {
    ...
    groups.push({id: category.id, icon: category.icon, settings: categorySettings});
  }

  const uncategorized = settings.filter((setting) => !used.has(setting.settingPanelId));
  if (uncategorized.length > 0) {
    groups.push({id: 'other', icon: OTHER_CATEGORY_ICON, settings: uncategorized});
  }
